...
def load_predictions():
    ...
    df = pd.read_sql(f"""
        SELECT bc.bracket_id, bc.bracket_name, bc.bracket_creator,
               bp.slot_id, ts.round, ts.region, ts.match_time,
    ...
    # Pre-convert match_time once at load time
...
def load_slots():
    ...
    return pd.read_sql(f"""
        SELECT ts.slot_id, ts.round, ts.region, ts.match_time,
               t1.team_name AS team_1, t2.team_name AS team_2,
...
def load_champs():
    conn = get_connection()
    df = pd.read_sql(f"""
        SELECT bc.bracket_name, bc.bracket_creator, t.team_name AS champion
