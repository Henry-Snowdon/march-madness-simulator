...
st.set_page_config(
    page_title="Who Should I Root For?",
    page_icon=None,
    layout="wide",
...
def load_sim_module():
    spec = importlib.util.spec_from_file_location(
        "sim32", os.path.join(os.path.dirname(__file__), "simulation_3_2.py"))
...
def main():
    ...
    with col_title:
        ...
        st.markdown('<div class="hero-sub1">A March Madness Pool Simulator</div>', unsafe_allow_html=True)
    ...
    # JS button: hidden when sidebar open, visible + functional when collapsed
    st.markdown("""
    ...
    if run_button:
        ...
        with st.spinner(f"Running {256 * n_sims:,} simulations..."):
